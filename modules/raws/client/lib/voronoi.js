...
(function() {
  ...
  var chart = raw.chart()
        ...
        .thumbnail('./modules/raws/client/img/voronoi.png')
  ...
  chart.draw(function(selection, data) {
    ...
    var x = d3.scale.linear().range([0, +width()]).domain(d3.extent(data, function(d) {
        return d.x;
      })),
      y = d3.scale.linear().range([+height(), 0]).domain(d3.extent(data, function(d) {
        return d.y;
      }));
    ...
    var path = g.selectAll('path')
            ...
            .style('fill', function(d) {
              return d && colors() ? colors()(d.point.color) : '#dddddd';
            })
            .style('stroke', '#fff')
            .attr('d', polygon);
