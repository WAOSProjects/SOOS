...
(function() {
  ...
  chart.draw(function(selection, data) {
    ...
    var groups = data,
      levels = d3.sum(d3.values(groups).map(function(d) {
        return d.length;
      })),
      marginLeft = raw.getMaxWidth(d3.entries(data), function(d) {
        return d.key;
      }),
      margin = {
        top: 10,
    ...
    colors.domain(values, function(d) {
      return d.color;
    });
    ...
    items.append('text')
            ...
            .attr('y', 0)
            .attr('dy', function(d) {
              return (d.value.length * itemHeight) / 2;
            })
